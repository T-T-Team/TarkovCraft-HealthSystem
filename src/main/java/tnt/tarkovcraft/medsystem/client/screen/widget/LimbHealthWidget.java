...
public class LimbHealthWidget extends AbstractWidget {
    ...
    public sealed interface DisplayComponent {
        ...
        final class IconDisplayComponent implements DisplayComponent {
            ...
            private void renderLimbName(GuiGraphics graphics, LimbHealthWidget parent, Limb limb) {
            ...
            private void renderLimbHealth(GuiGraphics graphics, LimbHealthWidget parent, Limb limb) {
            ...
            private void renderFullDisplay(GuiGraphics graphics, LimbHealthWidget parent, int hearts, int health, boolean vital) {
                for (int i = 0; i < hearts; i++) {
                    // background
                    int left = parent.getX() + parent.frameSize + 1 + i * 9;
                    int top = parent.getY() + parent.frameSize + 5;
                    graphics.blitSprite(Gui.HeartType.CONTAINER.getSprite(vital, false, false), left, top, 9, 9);

                    // health
                    int healthOffset = 2 * i;
                    if (health > healthOffset) {
                        boolean halfHeart = health - healthOffset == 1;
                        graphics.blitSprite(Gui.HeartType.NORMAL.getSprite(vital, halfHeart, false), left, top, 9, 9);
                    }
                }
            }

            private void renderSimplifiedDisplay(GuiGraphics graphics, LimbHealthWidget parent, int health, boolean vital) {
                int left = parent.getX() + parent.frameSize + 1;
                int top = parent.getY() + parent.frameSize + 5;
                graphics.blitSprite(Gui.HeartType.CONTAINER.getSprite(vital, false, false), left, top, 9, 9);
                graphics.blitSprite(Gui.HeartType.NORMAL.getSprite(vital, true, false), left, top, 9, 9);

                Component text = Component.literal(health + "x");
                graphics.drawString(parent.font, text, left + 12, top, ColorPalette.WHITE);
            }
        }
    }
}
