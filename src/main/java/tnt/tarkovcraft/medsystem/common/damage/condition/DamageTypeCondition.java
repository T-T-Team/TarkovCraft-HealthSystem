...
public final class DamageTypeCondition implements DamageCondition {
    ...
    public static final MapCodec<DamageTypeCondition> CODEC = ResourceLocation.CODEC.listOf()
            .xmap(DamageTypeCondition::fromIdList, condition -> condition.resourceKeys.stream().map(ResourceKey::location).toList()).fieldOf("values");
    ...
    public static DamageTypeCondition fromIdList(List<ResourceLocation> ids) {
        var keys = ids.stream()
