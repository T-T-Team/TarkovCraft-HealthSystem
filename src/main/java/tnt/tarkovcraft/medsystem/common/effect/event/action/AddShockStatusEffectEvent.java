package tnt.tarkovcraft.medsystem.common.effect.event.action;

import com.mojang.serialization.MapCodec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import net.minecraft.world.entity.LivingEntity;
import tnt.tarkovcraft.core.common.data.number.NumberProvider;
import tnt.tarkovcraft.core.common.data.number.NumberProviderType;
import tnt.tarkovcraft.medsystem.common.blood_system.BloodSystemManager;
import tnt.tarkovcraft.medsystem.common.blood_system.assignment.EntityBloodSystem;
import tnt.tarkovcraft.medsystem.common.blood_system.assignment.EntityBloodSystemDefinition;
import tnt.tarkovcraft.medsystem.common.effect.event.StatusEffectEventContext;
import tnt.tarkovcraft.medsystem.common.effect.event.function.StatusEffectEventFunction;
import tnt.tarkovcraft.medsystem.common.effect.event.function.StatusEffectEventFunctionType;
import tnt.tarkovcraft.medsystem.common.init.MedSystemStatusEffectEventActions;

import java.util.Collections;
import java.util.List;

public record AddShockStatusEffectEvent(List<StatusEffectEventFunction> scaleFunctions, NumberProvider amount) implements StatusEffectEventAction {

    public static final MapCodec<AddShockStatusEffectEvent> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            StatusEffectEventFunctionType.CODEC.listOf().optionalFieldOf("scale_functions", Collections.emptyList()).forGetter(AddShockStatusEffectEvent::scaleFunctions),
            NumberProviderType.VALUE_CODEC.fieldOf("amount").forGetter(AddShockStatusEffectEvent::amount)
    ).apply(instance, AddShockStatusEffectEvent::new));

    @Override
    public boolean apply(StatusEffectEventContext ctx) {
        LivingEntity entity = ctx.getEntity();
        if (BloodSystemManager.isEnabled(entity)) {
            float shockAmount = StatusEffectEventFunctionType.applyFunctions(this.amount.floatValue(), ctx, this.scaleFunctions);
            EntityBloodSystem bloodSystem = EntityBloodSystem.getAttached(entity);
            EntityBloodSystemDefinition definition = bloodSystem.getDefinition();
            float amount = definition.getReceivedShockValue(shockAmount, entity);
            bloodSystem.addShock(amount);
        }
        return true;
    }

    @Override
    public StatusEffectEventActionType<?> getType() {
        return MedSystemStatusEffectEventActions.ADD_SHOCK.value();
    }
}
