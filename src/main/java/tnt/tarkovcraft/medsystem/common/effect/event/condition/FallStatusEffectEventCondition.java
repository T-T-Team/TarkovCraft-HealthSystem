...
public record FallStatusEffectEventCondition(float minFallDistance, float modifier) implements StatusEffectEventCondition {
    ...
    public static final MapCodec<FallStatusEffectEventCondition> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            Codecs.NON_NEGATIVE_FLOAT.optionalFieldOf("min_fall_distance", 0.0F).forGetter(FallStatusEffectEventCondition::minFallDistance),
            Codecs.NON_NEGATIVE_FLOAT.optionalFieldOf("modifier", 1.0F).forGetter(FallStatusEffectEventCondition::modifier)
    ...
    public TriggerResult test(StatusEffectEventContext ctx) {
        ...
        float baseDistance = entity.fallDistance - this.minFallDistance;
        if (baseDistance <= 0) {
