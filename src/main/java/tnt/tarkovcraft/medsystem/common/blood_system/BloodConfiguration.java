...
public record BloodConfiguration(
    ...
    public static final Codec<BloodConfiguration> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.unboundedMap(Identifier.CODEC, Codecs.hashSet(Identifier.CODEC)).optionalFieldOf("compatibility_map", Collections.emptyMap()).forGetter(BloodConfiguration::compatibilityMap)
    ...
    public BloodTypeOptions getOptions(Identifier identifier) {
        return this.bloodTypes.get(identifier);
    }

    public boolean isCompatible(Identifier currentBloodType, Identifier compatibleWith) {
        if (currentBloodType.equals(compatibleWith))
            return true;
        Set<Identifier> compatibleTypes = this.compatibilityMap.getOrDefault(currentBloodType, Collections.emptySet());
