...
public final class StatusEffectArgument implements ArgumentType<StatusEffect> {
    ...
    private static final TagParser<?> DATA_PARSER = TagParser.create(NbtOps.INSTANCE);
    ...
    public StatusEffect parse(StringReader reader) throws CommandSyntaxException {
        ...
        return parseStatusEffect(DATA_PARSER, reader, effectType, this.registries);
    }
    ...
    private static <E extends StatusEffect, O> StatusEffect parseStatusEffect(TagParser<O> parser, StringReader reader, StatusEffectType<E> type, HolderLookup.Provider registries) throws CommandSyntaxException {
        RegistryOps<O> ops = registries.createSerializationContext(parser.getOps());
        O output;
        if (reader.canRead() && reader.peek() == '{') {
            output = parser.parseAsArgument(reader);
        } else {
            output = ops.emptyMap();
        }
        return type.mapCodec().codec().parse(ops, output)
