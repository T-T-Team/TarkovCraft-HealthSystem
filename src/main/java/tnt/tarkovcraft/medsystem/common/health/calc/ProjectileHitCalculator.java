    ...
    public HitCalculationResult calculateHits(HitCalculationContext context) {
        ...
        if (!hits.isEmpty()) {
            ...
            return HitCalculationResult.of(hits.subList(0, limit))
                    .withRayCast(ray)
                    .withDamageDistributor(prev -> new DecayingDamageDistributor(this.pierceDecay));
        }

        return context.approximate(ray).withDamageDistributor(prev -> new DecayingDamageDistributor(this.pierceDecay));
    }
}
